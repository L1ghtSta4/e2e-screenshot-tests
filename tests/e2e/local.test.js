...
test('Скріншот головної сторінки', async ({ page }) => {
  await page.goto('http://localhost:3000');
  expect(await page.screenshot()).toMatchSnapshot('screenshots/main-page.png');
});

test('Скріншот форми входу', async ({ page }) => {
  await page.goto('http://localhost:3000');
  const form = await page.locator('#loginForm');
  expect(await form.screenshot()).toMatchSnapshot('screenshots/login-form.png');
});